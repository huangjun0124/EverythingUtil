...
#define EVERYTHING_IPC_DRIVE											0x00000002	// the file or folder is a root.
...
#define EVERYTHING_IPC_NOTIFY_RESULTS_CHANGED							0 // results have changed, redraw all visible items. finish/abort current draw and redraw immediately.
#define EVERYTHING_IPC_NOTIFY_DB_CHANGED								1 // db has been changed, all queries are emptied, please execute a new search.
...
// The query is destroyed when the notify_hwnds thread ends or when your call EVERYTHING_IPC_WM_QUERY_DESTROY.
...
// SendMessage(everything_hwnd,EVERYTHING_IPC_WM_QUERY_SET_GET_RESULT_FILE_INFO_THUMBNAIL_MULTIPLY_ALPHA,(WPARAM)(HWND)notify_hwnd,(LPARAM)(COLORREF)multiple_alpha); 
// multiple_alpha should be in the range of 0 - 255, 0 being completely transparent to 255 being completely opaque.
// returns nonzero if successful. returns 0 if not supported.
// requires Everything 1.4
...
// use EVERYTHING_IPC_WM_QUERY_REQUEST_FILE_INFO to request information to be cached for certain results.
